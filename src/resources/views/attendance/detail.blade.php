    ...
    <div class="stamp-correction__table">
        <table class="stamp-correction__inner">
            <tr class="stamp-correction__item">
                ...
                <td class="text-item">
                    <p class="approval-name">{{ $latestApproval->user->name }}</p>
                </td>
            </tr>
            <tr class="stamp-correction__item">
                <th class="heading-item">日付</th>
                <td class="year-date">
                    <p class="top-item">{{ $latestApproval->detail->work_date->format('Y') }}年</p>
                    <p class="bottom-item">{{ $latestApproval->detail->work_date->format('n月j日') }}</p>
                </td>
            </tr>
            <tr class="stamp-correction__item">
                <th class="heading-item">出勤・退勤</th>
                <td class="date-time">
                    <p class="top-item">{{ $latestApproval->detail->start_work->format('H:i') }}</p>
                    <span class="time-separator">～</span>
                    <p class="bottom-item">{{ $latestApproval->detail->end_work->format('H:i') }}</p>
                </td>
            </tr>
            <tr class="stamp-correction__item">
                <th class="heading-item">休憩</th>
                <td class="date-time">
                    <p class="top-item">{{ $latestApproval->detail->rest_start->format('H:i') }}</p>
                    <span class="time-separator">～</span>
                    <p class="bottom-item">{{ $latestApproval->detail->rest_end->format('H:i') }}</p>
                </td>
            </tr>
            <tr class="stamp-correction__item">
                <th class="heading-item">休憩２</th>
                <td class="date-time">
                    <p class="top-item">{{ $latestApproval->detail->rest_start2?->format('H:i') }}</p>
                    @if($latestApproval->detail->rest_start2 && $latestApproval->detail->rest_end2)
                    <span class="time-separator">～</span>
                    @endif
                    <p class="bottom-item">{{ $latestApproval->detail->rest_end2?->format('H:i') }}</p>
                </td>
            </tr>
            <tr class="stamp-correction__item">
                <th class="heading-item">備考</th>
                <td class="text-item">
                    <p class="detail-reason">{{ $latestApproval->detail->reason }}</p>
                </td>
            </tr>
        </table>
    </div>
...
<div class="detail-form__content">
    ...
    <form action="{{ route('attendance.detail', $attendance->id) }}" method="post" class="form">
        ...
        <div class="form__button">
            @if($latestApproval && $latestApproval->status === 0)
